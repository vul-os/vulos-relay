...
Scope: Implement the `Sender` for standard recipients. NOTE (correction 2026-05-25): the transport is implemented with the standard library `net/smtp` (a hand-rolled SMTP transaction), NOT Mox `smtpclient` — the earlier scope/AC overstated the dependency. STARTTLS, a `required`/`opportunistic` TLS-downgrade policy, and **MTA-STS (RFC 8461) enforcement** are implemented natively in this repo (`internal/sending/smtp.go` + `internal/sending/mtasts.go`). Resolve recipient MX, deliver, classify the result into `SendResult{State: delivered|deferred|bounced, Code, EnhancedCode, Provider}`. Map 4xx → deferred (Nack), 5xx → bounced (Fail), success → delivered. Source-IP/HELO selection comes from the pool selector (RELAY-11) via a `SourceBinding` passed in; default to OS routing if none. No bounce-message generation here (that is the mail server's job) — just classify and return.
AC: [x] standard-library SMTP transport (`net/smtp`); MX resolved + transaction driven natively (Mox `smtpclient` NOT used) [x] delivered/deferred/bounced classified from SMTP reply codes [x] STARTTLS attempted; `required` policy refuses plaintext downgrade [x] **MTA-STS enforce honored**: enforce-mode recipient domains require TLS to a policy-matching MX with a valid cert, downgrade/mismatch defers (`internal/sending/mtasts.go`) [ ] **DANE/TLSA NOT implemented** (deferred — needs DNSSEC-validating resolver + TLSA verification; tracked separately) [x] SourceBinding selects HELO + source IP when provided [x] integration test against a local SMTP sink [x] go build ./... && go test ./internal/sending/...
