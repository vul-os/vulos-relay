...
| W2 | **Relocate gateway conformance + fuzz** envoir→wakala | IN PROGRESS | agent retrieving GWALIAS/GWATT/LEG/GWNAME cases + gateway_admission/gateway_alias fuzz from envoir 620a68c^, adapting to the wakala `gateway` crate (kotva_core, `gateway::` API) |
...
| W5 | **relay crate** (mesh, blind/structural) | IN PROGRESS | libp2p Circuit Relay v2 server wrapper + `RelayCoordinator` (blind/structural, self-host Backstop NOT scarce, no delivery path, no token); referencing envoir dmtap-p2p |
